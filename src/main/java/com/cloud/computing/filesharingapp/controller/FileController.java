...
public class FileController {
     ...
     * 
     * <p>This endpoint allows authenticated users to create shareable links for their files
     * with configurable permissions, expiration times, and optional email notifications.
     * 
     * @param fileId the ID of the file to share
     * @param shareRequest the share configuration parameters
     * @param authentication the current user's authentication context
     * @return ResponseEntity containing the ShareResponse with share details and URL
     */
        ...
        try {
            ...
            if (shareRequest.isSendNotification() && shareRequest.getRecipientEmails() != null 
                ...
                try {
                    ...
                    Optional<FileShare> fileShareOpt = fileSharingService.getFileShareById(shareResponse.getShareId(), user);
                    if (fileShareOpt.isPresent()) {
                        shareNotificationService.sendShareNotifications(fileShareOpt.get(), shareRequest.getRecipientEmails());
                        logger.info("Share notification emails sent for share ID: {}", shareResponse.getShareId());
                    } else {
                        logger.warn("Could not retrieve FileShare entity for notifications - ID: {}", shareResponse.getShareId());
                    }
            ...
            logger.error("Failed to create file share for user: {} - {}", userPrincipal.getUsername(), ex.getMessage(), ex);
            return ResponseEntity.badRequest().body(new MessageResponse("Could not create file share: " + ex.getMessage()));
        }
    }
     ...
     * 
     * <p>This endpoint allows anyone with a valid share token to access file information
     * and preview content. It performs comprehensive validation including token validity,
    ...
    public ResponseEntity<?> accessSharedFile(@PathVariable String token, HttpServletRequest request) {
        ...
        try {
            ...
            FileSharingService.ShareAccessValidationResult validation = 
                fileSharingService.validateShareAccess(token, clientIp, ShareAccessType.VIEW);
            ...
            return ResponseEntity.badRequest().body(new MessageResponse("Could not access shared file: " + ex.getMessage()));
        }
    }
     ...
     * 
     * <p>This endpoint allows users with valid download permissions to download shared files.
     * It performs comprehensive validation including permission checks and security controls.
     * 
     * @param token the share token
     * @param request the HTTP request for IP extraction
    ...
    public ResponseEntity<?> downloadSharedFile(@PathVariable String token, HttpServletRequest request) {
        ...
        try {
            ...
            FileSharingService.ShareAccessValidationResult validation = 
                fileSharingService.validateShareAccess(token, clientIp, ShareAccessType.DOWNLOAD);
            ...
            return ResponseEntity.badRequest().body(new MessageResponse("Could not download shared file: " + ex.getMessage()));
        }
    }
     ...
     * 
     * <p>This endpoint allows file owners to view all active shares for their files,
    ...
    public ResponseEntity<?> getFileShares(@PathVariable Long fileId, Authentication authentication) {
        ...
        try {
            ...
            logger.error("Failed to retrieve file shares for user: {} - {}", userPrincipal.getUsername(), ex.getMessage(), ex);
            return ResponseEntity.badRequest().body(new MessageResponse("Could not retrieve file shares: " + ex.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> updateShare(@PathVariable Long shareId,
        ...
        try {
            ...
            if (updatedShare.isEmpty()) {
                logger.warn("Share not found or access denied - ID: {}, user: {}", shareId, userPrincipal.getUsername());
                return ResponseEntity.notFound().build();
            }
     ...
     * 
     * <p>This endpoint provides a dashboard view of all files shared by the user,
     * including share statistics and access information.
     * 
     * @param authentication the current user's authentication context
    ...
    public ResponseEntity<?> getUserShares(Authentication authentication) {
        ...
        try {
            ...
            logger.error("Failed to retrieve user shares for user: {} - {}", userPrincipal.getUsername(), ex.getMessage(), ex);
            return ResponseEntity.badRequest().body(new MessageResponse("Could not retrieve shares: " + ex.getMessage()));
        }
    }
     ...
     */
    private HttpStatus getHttpStatusForDenial(com.cloud.computing.filesharingapp.service.ShareAccessService.AccessDenialType denialType) {
        if (denialType == null) {
    ...
    public static class SharedFileAccessResponse {
        ...
        public SharedFileAccessResponse(Long fileId, String originalFileName, String contentType, 
                                      Long fileSize, SharePermission permission, java.time.LocalDateTime expiresAt) {
            this.fileId = fileId;
